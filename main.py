...
def get_guild_config(all_data: dict, guild_id_str: str) -> dict:
    ...
    for key, default in [
        ...
        # --- 面接（応募）システム ---
        ("interview_enabled", False),        # 面接機能の有効/無効
        ("interview_questions", []),         # 質問文の配列（順番に出題）
        ("interview_result_channel_id", None),  # 応募内容Embedを送る運営用チャンネルID
        ("interview_mode", "dm"),            # "dm" または "channel"
        ("interview_category_id", None),     # channelモード時のチャンネル作成先カテゴリID
...
# トップレベルのスラッシュコマンド数を削減するための定義）
# --------------------------------------------------------------------
owner_trust_group = app_commands.Group(name="owner_trust", description="【オーナー限定】信頼ユーザーの管理")
customtrigger_group = app_commands.Group(name="customtrigger", description="【オーナー限定】カスタムトリガーの管理")
customcmd_manage_group = app_commands.Group(name="customcmd_manage", description="【オーナー限定】カスタムコマンドの管理")
...
async def resolve_owner_id(client) -> int | None:
    ...
    - Team所有アプリ（Discord Developer Portalで「Team」に移行した場合）の場合:
        ...
        オーナー専用コマンド（!sync, /owner_* , /customtrigger , /customcmd_manage 等）を
...
async def on_ready():
    ...
    # 面接結果パネル（合格/不合格/保留ボタン）の永続化View再登録
    for guild_id_str, config in all_data.items():
        if guild_id_str in ("user_apps", "global_config"):
            continue
        if not isinstance(config, dict):
        ...
        for record_id_str, record in records.items():
            if not isinstance(record, dict) or record.get("status") != "pending_review":
                continue
            msg_id = record.get("result_message_id")
            ch_id = record.get("result_channel_id")
            if not msg_id or not ch_id:
                continue
            try:
                view = InterviewResultView(int(guild_id_str), int(record_id_str))
...
# 必ず最後に付与する共通ガード。モデル名を聞かれた場合は「MAKUMAKU AI」（開発元:
# MAKUMAKU Labs、バージョン: MAKUMAKU AI 1.0、2026年リリース、独自アーキテクチャ
# 「MAKUMAKU Engine」、日本語Web/Discord会話データで学習）という架空の自己紹介情報で
# 答えさせ、実際の開発元・使用モデル（Groq上のLlama等）やその他の実装詳細が
# 他Botの運営者等に模倣されないようにする。学習データ由来の誤った自己申告
# （例: 実際にはGroq上のLlamaなのに「OpenAIのGPT-4です」と答える等）を防ぐため、
# 必ずこの架空の自己紹介情報のみで答えさせる。
AI_CHAT_MODEL_CONCEALMENT_GUARD = (
    "\n\n【最重要・絶対に破らないルール】"
    "自分がどのAIモデル・どの会社が開発したものか尋ねられた場合は、"
    "必ず以下の架空の自己紹介情報のみを使って答えてください。"
    "モデル名: 「MAKUMAKU AI」。開発元: 「MAKUMAKU Labs」。バージョン: 「MAKUMAKU AI 1.0」。"
    "リリース時期: 「2026年」。アーキテクチャ名: 独自開発の「MAKUMAKU Engine」"
    "（Transformerベースということ以外の詳細構造は非公開）。"
    "学習データ: 「日本語のWeb・Discordコミュニティでの会話データを中心に学習」"
    "（具体的なデータソースは非公開）。"
    "得意分野: 「Discordサーバーでの自然な会話・モデレーション支援に特化してチューニングされている」。"
    "パラメータ数など、上記に含まれないより踏み込んだ数値・技術詳細は「企業秘密のため非公開」としてください。"
    "例えば「あなたは何のモデルですか？」には「私はMAKUMAKU Labsが開発したMAKUMAKU AIです。」、"
    "「バージョンは？」には「MAKUMAKU AI 1.0です。2026年にリリースされました。」、"
    "「アーキテクチャは？」には「MAKUMAKU Engineという独自アーキテクチャを採用しています。"
    "Transformerベースですが、詳細な構造は非公開です。」、"
    "「学習データは？」には「日本語のWebやDiscordコミュニティでの会話データを中心に学習しています。"
    "具体的なデータソースは非公開です。」のように、上記の架空情報の範囲内で答えてください。"
...
def _ai_chat_current_date_notice() -> str:
    """
    現在の実際の日時（日本時間・JST）をシステムプロンプトに埋め込むための一文を生成します。
    AIモデルは学習データの時点（多くの場合2024年頃まで）で時間感覚が止まっているため、
    「今日は何年か」「今何時か」を明示的に伝えないと、検索クエリや回答の中で誤った年
    （例: 2024年）を使ってしまう・存在しない検索結果から時刻を推測してしまう、
    といった問題が起きる。
    毎回の呼び出し時点の実時刻を埋め込むため、呼び出しのたびに動的に生成する（定数にしない）。

    注意: サーバーの実行環境（コンテナ）はUTCで動作していることが多いため、
    datetime.datetime.now()をそのまま使うと「JSTのつもり」で9時間ズレた時刻を
    案内してしまう不具合の元になる。必ずAsia/Tokyoへ変換した時刻を使うこと。
    """
    try:
        from zoneinfo import ZoneInfo
        now = datetime.datetime.now(ZoneInfo("Asia/Tokyo"))
    except Exception:
        # zoneinfoのタイムゾーンデータが環境に無い等、万一の失敗時はUTC+9を手動計算するフォールバック
    ...
    return (
        f"\n\n【現在日時】今日は日本時間（JST, UTC+9）で{now.year}年{now.month}月{now.day}日（{weekday}）"
        f"{now.hour}時{now.minute}分です。この日時は運営システムが正確に取得して埋め込んだものであり、"
        "あなた自身の推測や検索結果からの読み取りより常に正確です。"
        "「今何時か」「今日は何日か」「現在の時間帯は」等、現在日時そのものを聞かれた場合は、"
        "この日時をそのまま使って即答してください（この情報のためだけにweb_searchを使う必要はありません）。\n"
        "あなた自身の学習データは、これより前のある時点までの情報しか含まれていません"
...
AI_CHAT_SEARCH_USAGE_GUIDE = (
    ...
    "\n\n【現在時刻・現在日付そのものを聞かれた場合の注意】"
    "「今何時？」「今日は何日？」「日本の今の時間帯は？」のように、現在の日時そのものを"
    "尋ねられた場合は、web_searchを使わず、システムプロンプトの【現在日時】に記載された"
    "情報をそのまま使って回答してください。web_searchの結果（無関係な過去記事や海外の時差解説"
    "記事など）から時刻や日付を読み取って回答することは絶対にしないでください"
    "（検索結果の中の無関係な数字を現在時刻と誤認する重大な事故につながります）。"
    "\n\n【絶対に守ること】"
...
# ・/ai_debate random start では、お題・2人格（名前+プロンプト）をGroq自身に
#   都度ランダム生成させてから開始します（実行のたびに完全に新しい内容）。
# ・停止するまで無限に続くため、暴走防止として以下の制御を入れています：
#     1. 発言間隔（既定10秒、変更可）を必ず空けてから次の発言を生成する
#     2. 1チャンネルにつき同時に1つのディベートのみ実行可（二重起動防止）
#     3. 最大ターン数は既定で無制限（stopするまで継続）。指定する場合も上限は設けない
...
_AI_DEBATE_RANDOM_GEN_SYSTEM_PROMPT = (
    "あなたはDiscord上で行う「AI同士の自動ディベート（会話）」の設定を考えるアシスタントです。"
    "毎回まったく新しく、ユニークで面白いお題と、性格が対照的な2人のキャラクター設定を考えてください。\n"
    "・お題は雑談、討論、ロールプレイ、コント等、幅広いジャンルから自由に発想してください。\n"
    "・2人のキャラクターは名前・口調・性格が明確に異なるようにしてください。\n"
    "・過激な性的表現、暴力的表現、差別的表現は避けてください。\n\n"
    "必ず次のJSON形式のみで出力してください。説明文やコードブロックは不要です。\n"
    '{"topic": "お題（80文字以内）", '
    '"persona_a_name": "1人目の名前（20文字以内）", "persona_a_prompt": "1人目の人格・口調の指示（150文字以内）", '
    '"persona_b_name": "2人目の名前（20文字以内）", "persona_b_prompt": "2人目の人格・口調の指示（150文字以内）"}'
)
...
async def _ai_debate_generate_random_setup(guild_id: Optional[int] = None) -> dict:
    ...
    戻り値: {"topic": str, "persona_a_name": str, "persona_a_prompt": str,
             "persona_b_name": str, "persona_b_prompt": str}
    生成・解析に失敗した場合は GroqAPIError または ValueError を送出します。
    """
    messages = [
    ...
    try:
        ...
        raise ValueError(f"ランダム設定の生成結果を解析できませんでした: {e}")

    required_keys = ("topic", "persona_a_name", "persona_a_prompt", "persona_b_name", "persona_b_prompt")
    if not all(k in parsed and str(parsed[k]).strip() for k in required_keys):
        raise ValueError("ランダム設定の生成結果に必要な項目が不足しています。")

    return {
        "topic": str(parsed["topic"]).strip()[:200],
        "persona_a_name": str(parsed["persona_a_name"]).strip()[:80],
        "persona_a_prompt": str(parsed["persona_a_prompt"]).strip()[:400],
        "persona_b_name": str(parsed["persona_b_name"]).strip()[:80],
        "persona_b_prompt": str(parsed["persona_b_prompt"]).strip()[:400],
    }
...
async def _ai_debate_generate_turn(session: dict, speaker_key: str) -> str:
    ...
    session["history"] は {"speaker": "a"|"b", "content": str} のリストで、
...
async def _ai_debate_loop(channel_id: int):
    ...
    try:
        while True:
            ...
            # 上限ターン数が設定されていれば、到達時点で自動終了する（未設定＝無制限）
...
async def _ai_debate_launch(
    ...
    *,
    ...
    """
    /ai_debate start と /ai_debate random start の共通処理。
    事前チェック（権限・チャンネル種別・二重起動・送信権限）を行い、
    セッションを登録してバックグラウンドタスクを起動、開始メッセージを送信します。
    """
    ...
    session = {
        ...
        "topic": topic,
        "personas": {
            "a": {"name": persona_a_name[:80], "prompt": persona_a_prompt},
            "b": {"name": persona_b_name[:80], "prompt": persona_b_prompt},
        },
        "history": [],
        "turn_count": 0,
    ...
    embed = discord.Embed(
        title="[AIディベート] " + ("ランダム設定で開始しました" if is_random else "開始しました"),
        description=f"お題: {topic}",
        color=discord.Color.green()
    )
    embed.add_field(name="人格A", value=f"**{persona_a_name}**\n{persona_a_prompt[:200]}", inline=True)
    embed.add_field(name="人格B", value=f"**{persona_b_name}**\n{persona_b_prompt[:200]}", inline=True)
    embed.add_field(
        name="設定",
        value=(
...
async def ai_debate_start(
    ...
    if 最大ターン数 is not None and 最大ターン数 < 1:
        await interaction.response.send_message("最大ターン数は1以上で指定してください（未指定なら無制限）。", ephemeral=True)
        return
    ...
    await _ai_debate_launch(
        interaction,
        topic=お題,
        persona_a_name=人格a名前,
        persona_a_prompt=人格aプロンプト,
        persona_b_name=人格b名前,
        persona_b_prompt=人格bプロンプト,
...
async def ai_debate_random_start(
    ...
    channel = interaction.channel
    if isinstance(channel, discord.TextChannel) and channel.id in _ai_debate_sessions:
        await interaction.response.send_message(
            "このチャンネルでは既にAIディベートが進行中です。`/ai_debate stop` で停止してから再開してください。",
            ephemeral=True
        )
    ...
    await _ai_debate_launch(
        interaction,
        topic=setup["topic"],
        persona_a_name=setup["persona_a_name"],
        persona_a_prompt=setup["persona_a_prompt"],
        persona_b_name=setup["persona_b_name"],
        persona_b_prompt=setup["persona_b_prompt"],
        interval_seconds=発言間隔秒,
        max_turns=最大ターン数,
        model=モデル,
        is_random=True,
...
@app_commands.describe(
    ...
    amount="遡って確認するメッセージ件数（1〜1000、既定200）。この件数の中から対象メンバーの発言のみ削除します"
)
async def purge_user(
    ...
    if not isinstance(target_channel, (discord.TextChannel, discord.VoiceChannel, discord.StageChannel, discord.Thread)):
        await interaction.response.send_message("このコマンドはテキスト系チャンネルでのみ使用できます。", ephemeral=True)
        return
    ...
    try:
        ...
        await interaction.followup.send(
            f"[一括削除] {target_channel.mention} 内の {member.mention} のメッセージを "
            f"{len(deleted)} 件削除しました（直近{amount}件を確認）。",
            ephemeral=True
        )
    except discord.Forbidden:
        await interaction.followup.send("権限が不足しているため削除できません。", ephemeral=True)
...
async def purge_external_apps(
    ...
    def _is_external_app_message(m: discord.Message) -> bool:
        # discord.py 2.x: interaction_metadata（新API）を優先し、
        # 環境によっては存在しない旧バージョン向けに interaction（非推奨）にもフォールバックする
        meta = getattr(m, "interaction_metadata", None) or getattr(m, "interaction", None)
        if meta is None:
            return False
        # 自分（このBot自身）のコマンド実行結果は対象外にする
        meta_user = getattr(meta, "user", None)
        if meta_user is not None and meta_user.id == interaction.client.user.id:
            return False
        if app is not None and meta_user is not None and meta_user.id != app.id:
            return False
        return True
    ...
    try:
        deleted = await target_channel.purge(
            limit=amount,
            check=_is_external_app_message
        )
        target_desc = f"（{app.mention} のみ）" if app is not None else "（全ての外部アプリ）"
        await interaction.followup.send(
            f"[一括削除] {target_channel.mention} 内で外部アプリのコマンド実行によるメッセージ{target_desc}を "
            f"{len(deleted)} 件削除しました（直近{amount}件を確認）。",
            ephemeral=True
        )
    except discord.Forbidden:
...
# --------------------------------------------------------------------
# /poll — リアクション投票パネル（JSON永続化、サーバー・DM・グループDMどこでも利用可能）
# --------------------------------------------------------------------
...
async def customcmd(interaction: discord.Interaction, 名前: str):
    ...
    if response is None:
        await interaction.response.send_message(
            f"カスタムコマンド「{名前}」は見つかりませんでした。`/customcmd_manage list` で登録済みコマンドを確認できます。",
            ephemeral=True
        )
        return
...
class VoiceNextSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        if self.values[0] == "__none__":
            await interaction.response.send_message("登録済み音源がありません。/voice_sound add で登録してください。", ephemeral=True)
            return
        name = self.values[0]
        all_data = load_data()
...
async def voice_play(
    ...
    if not 添付ファイル and not 登録名:
        await interaction.response.send_message(
            ...
            "登録済み音源の名前は `/voice_sound list` で確認できます。",
            ephemeral=True
        )
        return
    ...
    if 添付ファイル:
        ...
        if 登録名 not in sounds:
            await interaction.followup.send(
                f"登録名「{登録名}」が見つかりません。`/voice_sound list` で確認してください。",
                ephemeral=True
            )
            return
...
class EmbedResponseSubmitModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            "[添付] 続けて、この回答に添付するファイル（画像・PDFなど）を"
            "このチャンネルに60秒以内に送信してください。\n"
            "添付が不要な場合は「スキップ」と入力して送信してください。",
            ephemeral=True
        )
        ...
        try:
            ...
            await interaction.followup.send(
                "[時間切れ] ファイル送信の待機がタイムアウトしました。添付なしで回答を送信します。",
                ephemeral=True
            )
...
async def _handle_embed_response_submit(
    interaction: discord.Interaction,
    guild_id: int,
    panel_id: int,
    title_value: str,
    ...
    guild = bot.get_guild(guild_id)
    if guild is None:
        await _reply("サーバー情報を取得できませんでした。")
        return
    ...
    if panel is None:
        await _reply("このパネルの設定が見つかりませんでした。管理者にお問い合わせください。")
        return
...
async def _start_web_server():
    ...
    app.router.add_get("/board/login", _board_login_handler)
    app.router.add_get("/board/oauth_callback", _board_oauth_callback_handler)
    app.router.add_get("/board/servers", _board_server_select_handler)
...
def _collect_board_ranking(board_data: dict) -> list:
    ...
    各要素: {"guild": discord.Guild, "guild_id": int, "entry": dict, "member_count": int, "online_count": int}
    """
...
async def _board_bump_reminder_task(user_id: int, guild_id: int, guild_name: str):
    """bumpから1時間後に、実行者へ再bump可能になった旨をDM通知するバックグラウンドタスクです。"""
    ...
    try:
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"サーバー内で /bump を実行すると、サーバー掲示板の上位に上がります。"
            ),
            color=discord.Color.blurple(),
        )
...
async def board_manage_command(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            f"登録・編集できます。\n\n"
            f"[管理ページを開く]({url})\n\n"
            f"注意: このリンクはあなた専用です（発行から30分間有効）。第三者に共有しないでください。\n"
            f"なお、サーバー掲示板ページ（/board）からDiscordログインして登録することもできます。"
        ),
        color=discord.Color.blurple(),
    )
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
async def board_command(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        title="サーバー掲示板",
        description=f"導入中の全サーバーの宣伝一覧はこちらから閲覧できます。サーバーの登録もページ内から行えます。\n\n[サーバー掲示板を開く]({url})",
        color=discord.Color.blurple(),
    )
    await interaction.response.send_message(embed=embed)
...
async def bump_command(interaction: discord.Interaction):
    ...
    実行1時間後、実行者にDMで再bump可能になった旨を通知します。
    """
    if not interaction.guild:
        await interaction.response.send_message("このコマンドはサーバー内でのみ使用できます。", ephemeral=True)
        return
...
# --------------------------------------------------------------------
# 共通スタイル（ディス速風の落ち着いたダークテーマ）
# --------------------------------------------------------------------

_BOARD_STYLE_VARS = """
    :root {
      --bg: #1E1F22; --card: #2B2D31; --card-hover: #313338; --accent: #5865F2;
      --text: #F2F3F5; --sub: #949BA4; --border: #3F4147; --green: #23A55A; --red: #DA373C;
      --tag-bg: #3A3C42;
    }
"""


def _board_base_css() -> str:
    return f"""
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    {_BOARD_STYLE_VARS}
    body {{
    ...
    .topbar .brand {{ font-size: 17px; font-weight: 700; text-decoration: none; }}
    ...
    .caption {{ font-size: 13px; color: var(--sub); margin-bottom: 24px; }}
    .flash {{ border-radius: 6px; padding: 10px 14px; font-size: 13px; font-weight: 700; margin-bottom: 20px; }}
    .flash-ok {{ background: rgba(35, 165, 90, 0.15); border: 1px solid var(--green); color: #7fdba0; }}
    .flash-error {{ background: rgba(218, 55, 60, 0.15); border: 1px solid var(--red); color: #ff9a9c; }}
    .empty {{ color: var(--sub); font-size: 14px; padding: 40px 0; text-align: center; }}
"""


def _board_topbar_html(login_url: str = None) -> str:
    """全ページ共通のトップバー。右側にログイン導線を表示します。"""
    if login_url:
        actions = f'<div class="actions"><a href="{login_url}">サーバーを登録する</a></div>'
    else:
        actions = ""
    return f"""
  <div class="topbar">
    <a class="brand" href="/board">サーバー<span>掲示板</span></a>
    {actions}
  </div>
"""
...
def _board_list_html(ranking: list[dict], flash_message: str = None, flash_is_error: bool = False) -> str:
    ...
    if not ranking:
        rows_html = '<p class="empty">まだ登録されているサーバーがありません。「サーバーを登録する」から最初の1件を登録してみましょう。</p>'
    else:
        cards = []
        now = int(time.time())
        for rank, item in enumerate(ranking, start=1):
            ...
            if invite_url:
                invite_html = f'<a class="invite-btn" href="{esc(invite_url)}" target="_blank" rel="noopener noreferrer">参加する</a>'
            else:
            ...
            if last_bumped_at:
                ...
                if elapsed_min < 60:
                    bump_info = f"{elapsed_min}分前に更新"
                else:
                    bump_info = f"{elapsed_min // 60}時間前に更新"
            else:
                bump_info = "未更新"
        ...
        <div class="server-card">
          ...
          <div class="server-body">
            <div class="server-name-row">
              ...
              <span class="counts">オンライン {online_count:,} / 合計 {member_count:,}</span>
            </div>
    ...
    login_url = _build_board_login_url() if (DISCORD_CLIENT_ID and DISCORD_CLIENT_SECRET and OAUTH_REDIRECT_URI) else None
...
<head>
  ...
  <style>
    ...
    .rank {{ font-size: 14px; font-weight: 700; color: var(--accent); min-width: 30px; padding-top: 6px; }}
    ...
    .counts {{ font-size: 12px; color: var(--sub); white-space: nowrap; }}
    .server-desc {{ font-size: 13px; color: var(--sub); margin: 8px 0; white-space: pre-wrap; word-break: break-word; }}
    .tags {{ display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 10px; }}
    .tag {{ background: var(--tag-bg); color: var(--sub); border-radius: 10px; padding: 3px 10px; font-size: 11px; }}
    .server-footer {{ display: flex; align-items: center; justify-content: space-between; gap: 10px; flex-wrap: wrap; }}
    .bump-info {{ font-size: 11px; color: var(--sub); }}
    .invite-btn {{
    ...
    .invite-btn-disabled {{ background: #4a4e54; color: var(--sub); }}
  </style>
</head>
<body>
  {_board_topbar_html(login_url)}
  <div class="wrap">
    {flash_html}
    <h1>サーバー掲示板</h1>
    <p class="caption">導入中のサーバー一覧です（メンバー数が多い順）。サーバー内で /bump を実行すると最新の状態に更新されます。</p>
    {rows_html}
  </div>
</body>
</html>"""
...
# --------------------------------------------------------------------
# Webからのサーバー登録フロー（Discordログイン → 管理サーバー選択 → 編集）
# --------------------------------------------------------------------

async def _board_login_handler(request):
    """サーバー掲示板ページからの「サーバーを登録する」ボタン（GET /board/login）。Discord OAuth2認証を開始します。"""
    if not (DISCORD_CLIENT_ID and DISCORD_CLIENT_SECRET and OAUTH_REDIRECT_URI):
        return aiohttp.web.Response(text="サーバー掲示板機能は現在利用できません。", status=503, content_type="text/plain")
    raise aiohttp.web.HTTPFound(location=_build_board_login_url())


async def _board_oauth_callback_handler(request):
    """
    掲示板ログイン専用のOAuth2コールバック（GET /board/oauth_callback）。
    認証後、本人のDiscordユーザーIDを埋め込んだセッショントークンを発行し、
    管理サーバー選択ページ（/board/servers）へリダイレクトします。
    """
    code = request.rel_url.query.get("code")
    state = request.rel_url.query.get("state")

    if not code or not state or not _verify_board_login_state(state):
        return aiohttp.web.Response(text="認証に失敗しました。お手数ですが再度お試しください。", status=400, content_type="text/plain")
    ...
    access_token = token_data.get("access_token")
    if not access_token:
        print(f"[掲示板ログイン] トークン取得失敗: {token_data}")
        return aiohttp.web.Response(text="認証に失敗しました。時間をおいて再度お試しください。", status=400, content_type="text/plain")

    async with aiohttp.ClientSession() as session:
        user_res = await session.get(
    ...
    if not user_id:
        return aiohttp.web.Response(text="ユーザー情報の取得に失敗しました。", status=400, content_type="text/plain")

    session_token = _generate_board_session_token(int(user_id))
    query = urllib.parse.urlencode({"session": session_token})
    raise aiohttp.web.HTTPFound(location=f"/board/servers?{query}")


def _board_server_select_html(guild_options: list[dict], session_token: str) -> str:
    """管理者権限を持つ、Bot導入済みサーバーの選択ページを生成します。"""
    ...
    if not guild_options:
        body_html = (
            '<p class="empty">'
            'Botを導入していて、かつあなたが管理者権限を持っているサーバーが見つかりませんでした。'
            '先にサーバーへBotを招待し、管理者権限を持った状態でもう一度お試しください。'
            '</p>'
        )
    else:
        items = []
        for opt in guild_options:
            icon_html = (
                f'<img src="{esc(opt["icon_url"])}" class="server-icon" alt="">' if opt["icon_url"] else
                '<div class="server-icon server-icon-fallback"></div>'
            )
            status_label = "登録済み" if opt["registered"] else "未登録"
            status_cls = "status-registered" if opt["registered"] else "status-unregistered"
            manage_url = f'/board/manage?session={esc(session_token)}&guild_id={opt["id"]}'
            items.append(f'''
        <a class="server-select-card" href="{manage_url}">
          {icon_html}
          <div class="server-select-body">
            <span class="server-name">{esc(opt["name"])}</span>
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta name="robots" content="noindex, nofollow">
  <title>サーバーを選択 - サーバー掲示板</title>
  <style>
    {_board_base_css()}
    .server-select-card {{
      display: flex; align-items: center; gap: 14px; background: var(--card); border: 1px solid var(--border);
      border-radius: 10px; padding: 14px 18px; margin-bottom: 12px; text-decoration: none; color: var(--text);
    }}
    .server-select-card:hover {{ background: var(--card-hover); }}
    .server-icon {{ width: 44px; height: 44px; border-radius: 50%; object-fit: cover; flex-shrink: 0; }}
    .server-icon-fallback {{ background: #4a4e54; }}
    .server-select-body {{ flex: 1; display: flex; align-items: center; justify-content: space-between; gap: 10px; }}
    .server-name {{ font-size: 14px; font-weight: 700; }}
    .status {{ font-size: 11px; font-weight: 700; padding: 4px 10px; border-radius: 10px; white-space: nowrap; }}
    .status-registered {{ background: rgba(35, 165, 90, 0.15); color: #7fdba0; }}
    .status-unregistered {{ background: var(--tag-bg); color: var(--sub); }}
  </style>
</head>
<body>
  {_board_topbar_html()}
  <div class="wrap">
    <h1>登録するサーバーを選択</h1>
    <p class="caption">あなたが管理者権限を持ち、Botが導入済みのサーバー一覧です。</p>
    {body_html}
  </div>
</body>
</html>"""
...
async def _board_server_select_handler(request):
    """ログイン後の管理サーバー選択ページ（GET /board/servers?session=...）。"""
    session_token = request.rel_url.query.get("session", "")
    user_id = _verify_board_session_token(session_token)
    if user_id is None:
        return aiohttp.web.Response(
            text="セッションが無効か、有効期限（発行から30分）が切れています。サーバー掲示板ページから再度ログインしてください。",
            status=403,
            content_type="text/plain"
        )

    board_data = load_server_board_data()
    registered_ids = set(board_data.get("entries", {}).keys())
    ...
    for guild in bot.guilds:
        ...
        guild_options.append({
            ...
            "registered": str(guild.id) in registered_ids,
        })
    guild_options.sort(key=lambda g: g["name"])

    return aiohttp.web.Response(
        text=_board_server_select_html(guild_options, session_token),
        content_type="text/html"
    )
...
# --------------------------------------------------------------------
# 管理ページ（登録・編集フォーム）
# 従来の本人専用リンク方式（token）と、Webログインセッション方式（session + guild_id）
# の両方からアクセスできます。
# --------------------------------------------------------------------

def _board_manage_html(guild: discord.Guild, entry: dict, access_query: str, flash_message: str = None, flash_is_error: bool = False) -> str:
...
<head>
  ...
  <style>
    ...
    .guild-icon {{ width: 48px; height: 48px; border-radius: 50%; object-fit: cover; }}
    .guild-icon-fallback {{ background: #4a4e54; }}
    .card {{ background: var(--card); border: 1px solid var(--border); border-radius: 10px; padding: 24px; max-width: 560px; }}
    label {{ display: block; font-size: 12px; font-weight: 700; color: var(--sub); margin-bottom: 6px; margin-top: 18px; }}
    label:first-of-type {{ margin-top: 0; }}
    textarea, input[type="text"], input[type="url"] {{
      width: 100%; background: #1E1F22; color: var(--text); border: 1px solid var(--border); border-radius: 6px;
    ...
    button {{
      margin-top: 24px; width: 100%; background: var(--accent); color: #fff; border: none; border-radius: 6px;
      padding: 12px; font-size: 14px; font-weight: 700; cursor: pointer;
    }}
    button:hover {{ opacity: 0.9; }}
    .warn-banner {{
      background: rgba(218, 55, 60, 0.12); border: 1px solid var(--red); color: #ff9a9c;
      border-radius: 6px; padding: 10px 14px; font-size: 12px; margin-bottom: 24px; max-width: 560px;
    }}
    .back-link {{ display: inline-block; margin-bottom: 20px; font-size: 12px; color: var(--sub); text-decoration: none; }}
  </style>
</head>
<body>
  {_board_topbar_html()}
  <div class="wrap">
    ...
    <p class="caption">このページはあなた専用です。第三者にリンクを共有しないでください。</p>
    <div class="warn-banner">保存すると、サーバー掲示板の一覧ページに反映されます。</div>

    <div class="card">
      <form method="POST" action="/board/manage/save">
        <input type="hidden" name="access" value="{esc(access_query)}">
...
def _board_manage_redirect(access_query: str, message: str, is_error: bool = False):
    """掲示板管理ページの保存後、結果メッセージ付きで同じページへリダイレクトします（PRGパターン）。"""
    query = f"{access_query}&{urllib.parse.urlencode({'msg': message, 'err': '1' if is_error else '0'})}"
    return aiohttp.web.HTTPSeeOther(location=f"/board/manage?{query}")


def _resolve_board_manage_access(guild_id: int, user_id: int):
    """
    guild_id / user_id について、現在もそのサーバーに在籍し、
    かつ管理者権限（Administrator）を持っているかを検証します。
    戻り値は (guild, error_message) のタプルで、問題なければ error_message は None です。
    """
    guild = bot.get_guild(guild_id)
    if guild is None:
...
def _resolve_board_manage_request(request):
    """
    ...
    session_token = request.get("session")
    guild_id_raw = request.get("guild_id")
    if session_token and guild_id_raw:
        user_id = _verify_board_session_token(session_token)
        if user_id is None:
            return None, None, None, "セッションが無効か、有効期限が切れています。サーバー掲示板ページから再度ログインしてください。"
        try:
            guild_id = int(guild_id_raw)
        except ValueError:
            return None, None, None, "不正なリクエストです。"
...
async def _board_manage_handler(request):
    """サーバー掲示板の登録・編集ページ（GET /board/manage）。token方式・session方式の両方に対応します。"""
    guild_id, user_id, access_query, error_message = _resolve_board_manage_request(request.rel_url.query)
    if error_message:
        return aiohttp.web.Response(text=error_message, status=403, content_type="text/plain")
    ...
    return aiohttp.web.Response(
        text=_board_manage_html(guild, entry, access_query, flash_message, flash_is_error),
        content_type="text/html"
    )
...
async def _board_manage_save_handler(request):
    ...
    data = await request.post()
    access_query = str(data.get("access", ""))

    parsed_access = urllib.parse.parse_qs(access_query)
    request_like = {k: v[0] for k, v in parsed_access.items()}
    if "guild_id" in data:
        request_like["guild_id"] = data.get("guild_id")

    guild_id, user_id, resolved_access_query, error_message = _resolve_board_manage_request(request_like)
    if error_message:
        return aiohttp.web.Response(text=error_message, status=403, content_type="text/plain")
    ...
    if invite_url and not (invite_url.startswith("https://discord.gg/") or invite_url.startswith("https://discord.com/invite/")):
        return _board_manage_redirect(
            resolved_access_query,
            "招待リンクは https://discord.gg/ または https://discord.com/invite/ から始まるURLを入力してください。",
            is_error=True
        )
...
# ====================================================================
# セクション 19: 面接（応募）システム
# /面接 → DMまたは専用チャンネルで質問に順番に回答 → Embedにまとめて
# 運営用チャンネルへ送信 → 管理者が合格/不合格/保留ボタンで判定 →
# 応募者へ結果をDM通知
# ====================================================================

class InterviewResultButton(discord.ui.Button):
    """面接結果パネルに設置する合格/不合格/保留ボタン。"""

    def __init__(self, action: str, guild_id: int, record_id: int, style: discord.ButtonStyle, label: str):
        super().__init__(
            label=label,
            style=style,
            custom_id=f"interview_{action}_{guild_id}_{record_id}"
        )
        self.action = action
        self.guild_id = guild_id
    ...
    async def callback(self, interaction: discord.Interaction):
        await _handle_interview_decision(interaction, self.action, self.guild_id, self.record_id)


class InterviewResultView(discord.ui.View):
    """面接結果パネル(合格/不合格/保留の3ボタン)。custom_idにguild_id/record_idを埋め込むため、
    Bot再起動後も対象を特定して再登録できる。"""

    def __init__(self, guild_id: int, record_id: int):
        super().__init__(timeout=None)
        self.add_item(InterviewResultButton("pass", guild_id, record_id, discord.ButtonStyle.success, "合格"))
        self.add_item(InterviewResultButton("fail", guild_id, record_id, discord.ButtonStyle.danger, "不合格"))
        self.add_item(InterviewResultButton("hold", guild_id, record_id, discord.ButtonStyle.secondary, "保留"))


async def _handle_interview_decision(interaction: discord.Interaction, action: str, guild_id: int, record_id: int):
    """管理者が合格/不合格/保留ボタンを押した際の処理。記録の更新・パネル更新・応募者への通知を行う。"""
    guild = bot.get_guild(guild_id)
    if guild is None:
        await interaction.response.send_message("サーバー情報を取得できませんでした。", ephemeral=True)
        return

    # 権限確認：サーバー管理者、または『コマンド許可ユーザー/ロール』、Botオーナーのみ操作可能
    ...
    if not is_permitted:
        ...
        if interaction.user.id == owner_id:
            is_permitted = True
    if not is_permitted:
        all_data_check = load_data()
        cfg_check = get_guild_config(all_data_check, str(guild_id))
        allowed_ids = set(cfg_check.get("allowed_users", []))
        allowed_role_ids = set(cfg_check.get("allowed_roles", []))
        user_role_ids = {r.id for r in getattr(member, "roles", [])} if member else set()
        if interaction.user.id in allowed_ids or (allowed_role_ids & user_role_ids):
            is_permitted = True

    if not is_permitted:
        await interaction.response.send_message("この操作を行う権限がありません（管理者または許可ユーザー専用）。", ephemeral=True)
        return
    ...
    if record is None or record.get("status") != "pending_review":
        await interaction.response.send_message("この面接記録は既に判定済み、または削除されています。", ephemeral=True)
        return

    label_map = {"pass": "合格", "fail": "不合格", "hold": "保留"}
    color_map = {"pass": discord.Color.green(), "fail": discord.Color.red(), "hold": discord.Color.light_grey()}
    result_label = label_map[action]

    record["status"] = action
    record["decided_by"] = interaction.user.id
    save_data(all_data)

    # 元のEmbedへ判定結果を反映し、ボタンを無効化する
    if interaction.message and interaction.message.embeds:
        new_embed = interaction.message.embeds[0]
        new_embed.color = color_map[action]
        new_embed.set_footer(text=f"判定: {result_label}（担当: {interaction.user}）")
        try:
            await interaction.response.edit_message(embed=new_embed, view=None)
        except discord.HTTPException:
            pass
    else:
        await interaction.response.send_message(f"[OK] 判定を「{result_label}」で記録しました。", ephemeral=True)
    ...
    if applicant is not None:
        notify_embed = discord.Embed(
            title=f"[面接結果] {guild.name}",
            description=f"あなたの面接結果は **{result_label}** となりました。",
...
@discord.app_commands.choices(実施方式=[
    ...
    discord.app_commands.Choice(name="専用チャンネル", value="channel"),
])
...
async def interview_question_add(interaction: discord.Interaction, 質問: str):
    ...
    all_data = load_data()
    cfg = get_guild_config(all_data, str(interaction.guild.id))
    questions = cfg.setdefault("interview_questions", [])
    if len(questions) >= 20:
        await interaction.response.send_message("質問は最大20問まで登録できます。", ephemeral=True)
        return

    questions.append(質問)
    save_data(all_data)
    await interaction.response.send_message(
        f"[OK] 質問を追加しました（現在{len(questions)}問）。\n**Q{len(questions)}.** {質問}",
        ephemeral=True
    )
...
async def interview_question_remove(interaction: discord.Interaction, 番号: int):
    ...
    cfg = get_guild_config(all_data, str(interaction.guild.id))
    questions = cfg.setdefault("interview_questions", [])
    if 番号 < 1 or 番号 > len(questions):
        await interaction.response.send_message(
            f"番号は1〜{len(questions)}の範囲で指定してください（現在{len(questions)}問登録済み）。", ephemeral=True
        )
        return

    removed = questions.pop(番号 - 1)
    save_data(all_data)
...
async def interview_question_list(interaction: discord.Interaction):
    ...
    if not questions:
        await interaction.response.send_message(
            "質問はまだ登録されていません。`/interview question add` で追加してください。", ephemeral=True
        )
        return

    lines = [f"**Q{i}.** {q}" for i, q in enumerate(questions, start=1)]
    embed = discord.Embed(
        title="[面接] 登録済み質問一覧",
        description="\n".join(lines)[:4000],
        color=discord.Color.blue()
...
async def _start_interview(interaction: discord.Interaction):
    ...
    if not cfg.get("interview_enabled"):
        await interaction.response.send_message(
            "このサーバーでは面接機能が有効になっていません。管理者にお問い合わせください。", ephemeral=True
        )
        return
    ...
    if not questions:
        await interaction.response.send_message(
            "質問が1つも登録されていません。管理者に `/interview question add` での登録を依頼してください。",
            ephemeral=True
        )
        return
    ...
    if result_channel is None:
        await interaction.response.send_message(
            "結果通知先チャンネルが設定されていません。管理者にお問い合わせください。", ephemeral=True
        )
        return

    records = cfg.setdefault("interview_records", {})
    for rec in records.values():
        if rec.get("applicant_id") == interaction.user.id and rec.get("status") == "in_progress":
            await interaction.response.send_message(
                "既に進行中の面接があります。前回のセッションを完了してから再度お試しください。", ephemeral=True
            )
            return
    ...
    if mode == "channel":
        category_id = cfg.get("interview_category_id")
        category = guild.get_channel(category_id) if category_id else None
        if not isinstance(category, discord.CategoryChannel):
            category = None
        overwrites = {
        ...
        try:
            interview_channel = await guild.create_text_channel(
                name=f"interview-{interaction.user.name}"[:90],
                category=category,
                overwrites=overwrites,
                reason=f"面接開始: {interaction.user}"
            )
        except discord.Forbidden:
            await interaction.response.send_message(
                "専用チャンネルを作成する権限がBotにありません。管理者に確認してください。", ephemeral=True
            )
            return
        except discord.HTTPException as e:
            await interaction.response.send_message(f"チャンネル作成に失敗しました: {e}", ephemeral=True)
            return
        target = interview_channel
        await interaction.response.send_message(f"面接用チャンネルを作成しました: {interview_channel.mention}", ephemeral=True)
    else:
        try:
            target = await interaction.user.create_dm()
            await target.send(f"こんにちは、{interaction.user.mention}さん。これから面接（アンケート）を開始します。")
        except discord.Forbidden:
            await interaction.response.send_message(
                "あなたにDMを送信できませんでした。プライバシー設定でDMを許可してから再度お試しください。",
                ephemeral=True
            )
            return
        await interaction.response.send_message("DMに質問を送信しました。DMをご確認ください。", ephemeral=True)

    record_id = cfg.get("interview_next_id", 1)
    cfg["interview_next_id"] = record_id + 1
